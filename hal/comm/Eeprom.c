...
void eeprom_initConfig(TModuleConfig *moduleConfigL, TMotorConfig *motorConfigL)
{
	// initialize the eeprom
	if (eeprom_readConfigByte(TMCM_ADDR_EEPROM_MAGIC) != TMCM_EEPROM_MAGIC)
	{
		// clear eeprom (to know if erease has been interrupted before)
	...
	eeprom_readConfigBlock(TMCM_ADDR_MODULE_CONFIG, (uint8_t *)moduleConfigL, sizeof(TModuleConfig));
